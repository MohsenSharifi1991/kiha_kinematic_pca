...
def wandb_scatter_2d_train_test(train_pca, test_pca, train_labels, test_labels, status_title):
    ...
    train_subjects = train_labels['subjects'].unique()
    label_activity = train_labels['activity'].unique()
    type_activity = train_labels['phase_1'].unique()
    ...
    if train_single_color:
        ...
        for s, label in enumerate(label_activity):
            for t, type in enumerate(type_activity):
                c = train_colors[i]
                index = train_labels[(train_labels['activity'] == label) & (train_labels['phase_1'] == type)].index.values
        ...
        for s, train_subject in enumerate(train_subjects):
            index = np.where(train_labels['subjects'] == train_subject)[0]
            c = train_colors[s]
