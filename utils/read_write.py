...
def read_xsens_imus(xsens_imu_folder):
    s_indx = xsens_imu_folder.index("S")
    subject_num = xsens_imu_folder[s_indx:s_indx+3]
    imu = {}
...
def read_information_subject(filename):
    data = pd.read_excel(filename, sheet_name='Machine Learning Format', index_col=False)
    return data
...
def read_matfile(filename):
    ...
    try:
        ...
        data = data['activity_stair_index']
